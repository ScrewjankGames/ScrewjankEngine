...
public:
    ...
    constexpr dynamic_array(const Allocator& alloc) : m_allocator(alloc)
    {
    }

    constexpr dynamic_array(size_type capacity, const Allocator& alloc = Allocator())
        : m_allocator(alloc)
    {
        resize(capacity);
    }

    constexpr dynamic_array(size_type capacity,
    ...
    constexpr dynamic_array(std::initializer_list<T> elems)
    ...
    constexpr dynamic_array(std::from_range_t,
                            std::ranges::range auto&& rg,
                            const Allocator& alloc = Allocator())
        : m_allocator(alloc), m_capacity(std::size(rg))
    {
        m_array = m_allocator.allocate(m_capacity);

        for(auto&& [idx, elem] : std::views::enumerate(rg))
            new(std::addressof(m_array[idx])) T(std::forward<T>(elem));
    }
