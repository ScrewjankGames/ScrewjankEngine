...
namespace system_tests {
    ...
    struct FreeListAllocatorDummy
    {
        char Label;
        double Value;
    };

    TEST(PoolAllocatorTests, BasicAllocationTest)
    {
        PoolAllocator<sizeof(FreeListAllocatorDummy)> allocator(
            4,
            MemorySystem::GetDefaultUnmanagedAllocator());

        auto mem_loc1 = allocator.AllocateType<FreeListAllocatorDummy>();
        ASSERT_NE(nullptr, mem_loc1);
        auto dummy1 = new (mem_loc1) FreeListAllocatorDummy {'a', 3.14};

        auto mem_loc2 = allocator.AllocateType<FreeListAllocatorDummy>();
        ASSERT_NE(nullptr, mem_loc2);
        auto dummy2 = new (mem_loc2) FreeListAllocatorDummy {'b', 3.14};

        auto mem_loc3 = allocator.AllocateType<FreeListAllocatorDummy>();
        ASSERT_NE(nullptr, mem_loc3);
        auto dummy3 = new (mem_loc3) FreeListAllocatorDummy {'c', 3.14};

        auto mem_loc4 = allocator.AllocateType<FreeListAllocatorDummy>();
        ASSERT_NE(nullptr, mem_loc4);
        auto dummy4 = new (mem_loc4) FreeListAllocatorDummy {'d', 3.14};

        // Should not be able to allocate a 5th block
        ASSERT_EQ(nullptr, allocator.AllocateType<FreeListAllocatorDummy>());
        ...
        allocator.Delete<FreeListAllocatorDummy>(dummy3);
        ...
        mem_loc3 = allocator.AllocateType<FreeListAllocatorDummy>();
        ASSERT_NE(nullptr, mem_loc3);
        dummy3 = new (mem_loc3) FreeListAllocatorDummy {'c', 3.14};
